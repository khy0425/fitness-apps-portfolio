...
class _SplashScreenState extends State<SplashScreen>
  ...
  void _startAnimation() async {
    ...
    try {
      ...
      // 1단계: 온보딩 완료 여부 확인 (개발 중에는 건너뛰기)
      bool isOnboardingCompleted = true; // 개발 편의성을 위해 true로 설정
      debugPrint('개발 모드: 온보딩 건너뛰기');
      
      // 2단계: 권한 확인 (개발 중에는 건너뛰기)
      final hasAllPermissions = true; // 개발 편의성을 위해 true로 설정
      debugPrint('개발 모드: 권한 확인 건너뛰기');
      
      // 3단계: UserProfile 생성 여부 확인 (개발 중에는 건너뛰기)
      bool hasUserProfile = true; // 개발 편의성을 위해 true로 설정
      debugPrint('개발 모드: UserProfile 확인 건너뛰기');
